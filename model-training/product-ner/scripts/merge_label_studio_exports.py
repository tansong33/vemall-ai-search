...
def merge_exports(
    ...
    *,
    ...
    for tasks in exports:
        ...
        for task in tasks:
            ...
            if query_id in seen_in_export:
                raise ValueError(
                    f"duplicate query {query_id} in one personal export"
                )
            seen_in_export.add(query_id)
            label_studio.choose_annotation(task)
    ...
    for query_id in sorted(grouped):
        ...
        annotations = [
            label_studio.choose_annotation(task) for task in candidates
        ]
        entity_sets = [
            label_studio.entity_keys(
                label_studio.entities_from_annotation(task, annotation)
            )
        ...
        if len(candidates) == 1:
            ...
            entity_sets[0] == entity_set for entity_set in entity_sets[1:]
        ):
            selected_task = copy.deepcopy(candidates[0])
            selected_annotation = copy.deepcopy(annotations[0])
            selected_annotation["ground_truth"] = True
            selected_task["data"]["approved_by"] = (
            ...
            if query_id not in adjudicated:
                raise ValueError(
                    f"query {query_id} has conflicting annotations "
                    "but no adjudication"
                )
            selected_task = copy.deepcopy(adjudicated[query_id])
            selected_annotation = copy.deepcopy(
                label_studio.choose_annotation(selected_task)
            )
            if selected_task["data"]["text"] not in texts:
                raise ValueError(
                    f"query {query_id} adjudication text does not match source"
                )
            selected_annotation["ground_truth"] = True
            selected_task["data"]["approved_by"] = (
                approved_by.strip() or "product-lead"
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--input", required=True, action="append", type=_input_path
    )
    ...
    personal_exports = [
        label_studio.read_export(path) for path in args.input
    ]
    adjudication = (
        label_studio.read_export(args.adjudication)
    ...
    label_studio.write_tasks(args.output, merged)
    print(
        json.dumps(
