#!/usr/bin/env python3
"""Deterministically split Label Studio tasks into personal import files."""
...
def _assigned_copy(
    ...
    *,
    ...
    if review_pair_id:
        data["review_pair_id"] = review_pair_id
    else:
        data.pop("review_pair_id", None)
    return assigned
    ...
    *,
    ...
    for task in tasks:
        ...
        if not isinstance(text, str) or not text.strip():
            raise ValueError(f"task {query_id} has an empty data.text")
    ...
    for index, (query_id, task) in enumerate(indexed):
        ...
        review_pair_id = f"overlap:{query_id}" if is_overlap else ""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Split generated Label Studio JSON tasks among annotators."
    )
